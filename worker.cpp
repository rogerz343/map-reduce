...
Worker::Worker(std::string worker_name, std::string server_ip, std::string server_port,
        std::string map_exec, std::string reduce_exec,
        int wait_time = 10) :
        worker_name(worker_name), server_ip(server_ip), server_port(server_port),
        map_exec(map_exec), reduce_exec(reduce_exec),
        wait_time(wait_time) {
}

bool Worker::send_to_server(int sockfd, const std::string &msg) {
...
int Worker::run_client() {
    ...
    if (!send_to_server(sockfd, worker_name)) {
        close(sockfd);
        freeaddrinfo(servinfo);
        return 1;
    }
